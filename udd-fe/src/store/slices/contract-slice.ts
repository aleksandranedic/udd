...
import { advancedSearch, basicSearch, geoSearch, indexContract, indexLaw, submitContract, submitLaw } from "../actions/contract-actions";
...
export const contractSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(geoSearch.pending, (state: ContractState) => {
      state = { ...state, loading: true };
      return state;
    });
    builder.addCase(geoSearch.fulfilled, (state: ContractState, action) => { 
      state.searchResults = action.payload.content;
      state.page = action.payload.page;
      state.size = action.payload.size;
      state.total = action.payload.total;
    ...
    builder.addCase(geoSearch.rejected, (state: ContractState) => {
      state = {...state, loading: false};
      return state;
    });
  },
